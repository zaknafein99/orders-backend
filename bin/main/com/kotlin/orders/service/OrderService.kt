...
class OrderService(
    ...
    fun createOrder(orderDTO: OrderDTO): OrderDTO {
        ...
        // Get a default truck if none is provided
        val truck = if (orderDTO.truck?.id != null) {
            truckRepository.findById(orderDTO.truck.id!!).orElseThrow {
                EntityNotFoundException("Truck not found with ID ${orderDTO.truck.id}")
            }
        } else {
            // Get the first available truck or throw an exception
            truckRepository.findAll().firstOrNull() ?: throw EntityNotFoundException("No trucks available")
        }
